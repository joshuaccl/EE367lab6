...
void init_forwarding_table(struct forwarding table[100]){
    for(int i = 0; i < 100; i++){
        table[i].port_id = i;
        table[i].valid = false;   // initialize all invalid
...
int find_host_in_table(struct forwarding table[100], int host_id){
    ...
    for(int i = 0; i < 100; i++){
        if(table[i].dest_id==host_id && table[i].valid==true)
            return i;
    }
    return -1;
}
...
void switch_main(int switch_id)
{
    ...
    // struct packet *new_packet; /* we do not need to make a new packet */
    ...
    while(1) {
        ...
        */
        ...
        if (job_q_num(&job_q) > 0) {
            ...
            if( find_host_in_table(f_table, new_job->packet->dst)==-1)  {
                //send packet on all ports except the src port
                for (k=0; k<node_port_num; k++) {
                    if(new_job->packet->src != k )
                        packet_send(node_port[k], new_job->packet);
                }
            }  
            else {
                packet_send(node_port[find_host_in_table(f_table, new_job->packet->dst)], new_job->packet);
            }
                free(new_job->packet);
                free(new_job);
                break;
